...
from app.nse_helper import get_stock_quote, get_index_quote, search_stock_by_prefix, clean_symbol
...
def quick_price(symbol: str):
    ...
    is_index = sym in ("^NSEI", "NIFTY", "^NSEBANK", "BANKNIFTY")
    try:
        if is_index:
            data = get_index_quote(sym)
            return {
                "symbol":        symbol.upper(),
                "company_name":  data.get("index", sym),
                "current_price": round(float(data.get("last", 0)), 2),
                "sector":        "Index"
            }
        else:
            ...
            return {
                ...
                "company_name":  data.get("companyName", sym),
                "current_price": round(float(data.get("lastPrice", 0)), 2),
...
def search_stocks(query: str):
    results = search_stock_by_prefix(query)
